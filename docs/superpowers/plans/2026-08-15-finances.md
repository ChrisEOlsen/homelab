...
- Scaffold: `scaffold_resource(name='client', fields=[...])` — generates `models/Client.go`, `handlers/clients.go`, `static/pages/clients.html`, `static/js/clients.js`, and registers 5 routes
...
Confirm `src/app/models/Client.go`, `src/app/models/Client_test.go`, `src/app/handlers/clients.go` exist, and that `src/app/api.json` now lists a `Client` model and five `/api/v1/clients` endpoints.
...
Confirm `src/app/models/RateRule.go` and `src/app/handlers/rate_rules.go` exist and `api.json` lists five `/api/v1/rate_rules` endpoints.
...
Confirm `src/app/models/TrainingSession.go` and `src/app/handlers/training_sessions.go` exist and `api.json` lists five `/api/v1/training_sessions` endpoints.
...
- Modify: `src/app/handlers/expenses.go` — marking `bought` stamps `incurred_on`
...
In `src/app/handlers/expenses.go`, replace the body of `ExpenseUpdatePUT` between decoding the request and calling `model.Update` with:
...
- Modify: `src/app/handlers/subscriptions.go` — deactivating stamps `ended_on`
...
In `src/app/handlers/subscriptions.go`, insert immediately after the decode in `SubscriptionUpdatePUT`:
